...
The Nginx gateway terminates TLS using a self-signed certificate generated by
`setup-secrets.sh` (`secrets/web_tls_cert.pem` / `secrets/web_tls_key.pem`).
Replace those with a certificate from a trusted CA before production use and
accept the browser warning for local development, or trust the cert once:

```bash
sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain secrets/web_tls_cert.pem
```
...
# Data-access policy (row restrictions / column masking) as a JSON array.
# An empty list ("[]") boots with no restrictions; production should define
# real policies. Fail-fast at startup if absent in production.
export POLICY_POLICIES_JSON=<json-array>
# or mount a secret file and reference it:
export POLICY_POLICIES_JSON_FILE=/run/secrets/policy_policies.json
