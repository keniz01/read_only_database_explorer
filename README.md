...
`scripts/bootstrap-dev.sh` (`certs/web_tls_cert.pem` / `certs/web_tls_key.pem`).
...
```bash
sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain certs/web_tls_cert.pem
```
